...
Using ``pragma abicoder v1`` or ``pragma abicoder v2`` you can
select between the two implementations of the ABI encoder and decoder.

The new ABI encoder (v2) is able to encode and decode arbitrarily nested
...
.. note::
  ...
  using ``pragma experimental ABIEncoderV2``, but it was not possible
...
Because the ABI coder version 2 is not considered experimental anymore,
